...
4. **Deployment** If user selects the option to go ahead with the deployment ,then use skill : **GFE-Managed-deploymnet-Skill.md** to finish the deployment. 
...
* **Next Action:** Ask the user to choose their deployment/generation format (Terraform HCL or gcloud CLI Bash Script) and their next action:
  ...
  3. **Deploy Configuration** Initiate the deployment via Infrastructure Manager or execute the gcloud script. This should be done usil skill : **GFE-Managed-deployment-Skill.md**
